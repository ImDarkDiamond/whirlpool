...
class Admin(commands.Cog):
    ...
    async def sql_table(self, ctx, *, table_name: str):
        ...
        results = await self.bot.pool.fetch(query, table_name)
...
def setup(bot):
    ...
    async def sh(self, ctx, *, command):
        ...
        from ..utilities.paginator import TextPageSource, RoboPages
...
# class ComfTrue(discord.ui.Button):
#     def __init__(self, ctx):
#         super().__init__(style=discord.ButtonStyle.success, label="Continue")
#         self.ctx = ctx

#     async def callback(self, interaction: discord.Interaction):
#         assert self.view is not None
#         view: Button2() = self.view

#         self.disabled = True
#         view.stop()
#         await interaction.channel.send(content=f"{interaction.user.id == self.ctx.author.id}")


# class ComfFalse(discord.ui.Button):
#     def __init__(self, ctx):
...
#     def __init__(self, ctx):
#         super().__init__()
#         self.add_item(ComfTrue(ctx=ctx))
#         self.add_item(ComfFalse(ctx=ctx))